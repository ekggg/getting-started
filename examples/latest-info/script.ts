...
EKG.widget("LatestInfo")
  .initialState<State>((ctx, initialData) => {
    ...
    if (infoType === "followerName" && initialData.latestFollower) {
      return {
        type: "text",
        text: initialData.latestFollower.data.userDisplayName,
      };
    }
    ...
    if (infoType === "subscriberName" && initialData.latestSubscriber) {
      return {
        type: "text",
        text: initialData.latestSubscriber.data.userDisplayName,
      };
    }
    ...
    if (infoType === "tipperName" && initialData.latestTip) {
      return {
        type: "text",
        text: initialData.latestTip.data.userDisplayName,
      };
    }
  ...
  .register((event, state, ctx) => {
    ...
    if (event.type === "ekg.channel.followed" && infoType === "followerName") {
      return { type: "text", text: event.data.userDisplayName };
    }
    ...
    if (
      ...
      return {
        type: "text",
        text: event.data.userDisplayName ?? "Anonymous",
      };
    }
