...
EKG.widget("LatestSupporters")
  .initialState<State>((ctx, initialData) => ({
    latestFollower: initialData.latestFollower
      ? {
          name: initialData.latestFollower.data.userDisplayName,
    ...
    latestSubscriber: initialData.latestSubscriber
      ? {
          name: initialData.latestSubscriber.data.userDisplayName,
    ...
    latestTip: initialData.latestTip
      ? {
          name: initialData.latestTip.data.userDisplayName,
    ...
    switch (event.type) {
      case "ekg.channel.followed": {
        const isNew =
          ...
          state.latestFollower.name !== event.data.userDisplayName;
        return {
          ...state,
          latestFollower: {
      ...
      case "ekg.subscription.started": {
        const isNew =
          ...
          state.latestSubscriber.name !== event.data.userDisplayName;
        return {
          ...state,
          latestSubscriber: {
            name: event.data.userDisplayName,
      ...
      case "ekg.tip.sent": {
        const isNew =
          !state.latestTip ||
          state.latestTip.name !== event.data.userDisplayName ||
      ...
      case "ekg.subscription.renewed": {
        const isNew =
          !state.latestResub ||
          state.latestResub.name !== event.data.userDisplayName ||
