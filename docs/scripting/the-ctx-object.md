...
`.initialState()` function and every call of your `.register()` handler,
providing essential data that your widget needs to function properly.

## Available properties

The `ctx` object contains several key properties that give your widget access to
its environment. The `settings` property contains all user-configured settings
for your widget, allowing users to customize behavior, colors, and other
preferences. The `assets` property provides access to developer-uploaded images
and other media files. The `now` property gives you the current timestamp in
milliseconds, essential for time-based functionality. The `size` property
contains the current dimensions of your widget on the page, and the `random()`
function provides seeded pseudorandom numbers for consistent behavior.
...
`ctx.settings.settingName`, where `settingName` matches the keys defined in your
widget's manifest file. For example, if you defined a setting called
...
See [Settings Types](../settings/list-of-types.md) for details on available
types and what value your widget will receive.
...
under the keys you defined in your manifest file. For example, if you defined an
asset called `background-img` in your manifest, you can access its URL with
...
See [Asset Types](../assets/list-of-types.md) for details on available types and
what value your widget will receive.
...
screen sizes and OBS configurations. Access the dimensions with `ctx.size.width`
and `ctx.size.height`, both provided in pixels. Your widget will automatically
receive resize events when the container size changes, giving you the
opportunity to recalculate layouts or adjust content based on available space.
